...
#define CONDITIONAL(CONDITION, VALUE) ((CONDITION)?(VALUE):none_text)
		...
		string start_hour   =CONDITIONAL (f.flight_starts_here () && f.started, f.launchTime.to_string ("%H", tz_utc, 3, true));
		string start_minute =CONDITIONAL (f.flight_starts_here () && f.started, f.launchTime.to_string ("%M", tz_utc, 3, true));
		string land_hour    =CONDITIONAL (f.flight_lands_here () && f.landed,   f.landingTime.to_string ("%H", tz_utc, 3, true));
		string land_minute  =CONDITIONAL (f.flight_lands_here () && f.landed,   f.landingTime.to_string ("%M", tz_utc, 3, true));
#undef CONDITION

		// Flugart (siehe unter Flugart)
		string flight_type=error_text;
		QString accounting_note=(std2q (f.accountingNote));
...
#undef HANDLE_PERSON
		...
		else if (label==field_name_bemerkungen) (*field).set_to (f.comments);
		else if (label==field_name_abrechnungshinweis) (*field).set_to (f.accountingNote);
